...
function leftClick(e: MouseEvent) {
  btnControlAnimation(() => index.value <= 0, -1, 'left')
}
// swiper 右边控制按钮
function rightClick(e: MouseEvent) {
  btnControlAnimation(() => index.value === count.value, 1, 'right')
}

// swiper 俩侧控制动画逻辑
function btnControlAnimation(fn: () => Boolean, step: number, direction: 'left' | 'right') {
  ...
  //     index.value = i
  //   }
  //   i = count.value - index.value === 0 ? count.value - 1 : count.value - index.value
  //   leftValue = index.value * domInfo.value.Width
  // } else if (index.value === count.value && direction === 'right') {
  //   index.value = 0
  //   childrens[0].setOffset(index.value * domInfo.value.Width * step)
  // } 
  
  // childrens[i].setOffset((i + 1) * domInfo.value.Width * step)
  // left_distance.value = leftValue
  let currentIndex
  if (index.value <= -1) {
    index.value = Math.min(Math.max(index.value + count.value, -1), count.value);
  ...
  console.log(index.value * domInfo.value.Width)
